		...
		try {
			...
			{	
				if( this.Outputs[Stencil.TOP] != null){
					this.Outputs[Stencil.TOP].sendObj(LocalData.getTop());
				}
				//right
				if( this.Outputs[Stencil.RIGHT] != null){
					this.Outputs[Stencil.RIGHT].sendObj(LocalData.getRight());
				}
				//bottom
				if( this.Outputs[Stencil.BOTTOM] != null){
					this.Outputs[Stencil.BOTTOM].sendObj(LocalData.getBottom());
				}
				//left
				if( this.Outputs[Stencil.LEFT] != null){
					this.Outputs[Stencil.LEFT].sendObj(LocalData.getLeft());
				}
			}
			//take input
			{
			...
			if( this.getCycleVersion() == 2000 ){
				benchmark_start = System.currentTimeMillis();
			}
			
			if( this.getCycleVersion() == 2500 ){
				FileWriter w = new FileWriter( "./five_hundred_cycles_segment"+this.getSegment()+".txt", true);
