package edu.uncc.grid.seeds.dataflow.pattern.stencil.perfectsquare;
...
import edu.uncc.grid.seeds.dataflow.pattern.stencil.perfectsquare.Stencil;
...
public class LoadStencil extends DataflowLoader {
	...
	public LoadStencil(Stencil mod){
		...
		//row col
		//height width
		DataflowMatrix = new StencilDataflow[height][width];
		
		int segment = 0;
		for( int r = 0; r < height ; r++){
			for(int c = 0; c < width; c++){
				DataflowMatrix[r][c] = new StencilDataflow();
				HierarchicalSegmentID seg = new HierarchicalSegmentID(segment , 1);
				DataflowMatrix[r][c].setSegID(seg);
				...
				DataflowMatrix[r][c].setOutputDependencyIDs(outs);
				++segment;
			}
		}
		...
		for( int r = 0; r < height ; r++){
			for(int c = 0; c < width; c++){
				...
				if( r-1 >= 0 ){
					ins[Stencil.TOP] = DataflowMatrix[r-1][c].getOutputDependencyIDs()[Stencil.BOTTOM];
				}else{
					ins[Stencil.TOP] = null;
				}
				if( c+1 < width){
					ins[Stencil.RIGHT] = DataflowMatrix[r][c+1].getOutputDependencyIDs()[Stencil.LEFT];	
				}else{
					ins[Stencil.RIGHT] = null;
				}
				if( r+1 < height){
					ins[Stencil.BOTTOM] = DataflowMatrix[r+1][c].getOutputDependencyIDs()[Stencil.TOP];
				}else{
					ins[Stencil.BOTTOM] = null;
				}
				if( c-1 >= 0 ){
					ins[Stencil.LEFT] = DataflowMatrix[r][c-1].getOutputDependencyIDs()[Stencil.RIGHT];
				}else{
					ins[Stencil.LEFT] = null;
				}
				DataflowMatrix[r][c].setInputDependencyIDs(ins);
			}
		}
	}
	...
	public Dataflow onLoadPerceptron(int segment) {
		...
		DataflowMatrix[r][c].setState( state );
		DataflowMatrix[r][c].CellSegment = segment;
		return DataflowMatrix[r][c];
	}
